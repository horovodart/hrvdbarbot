# Бар HOROVOD HUB — Telegram Mini App

Склад напитков скушобни: остатки, средний расход, цена закупки, закупки и подсчёт раз в 2 недели.
Интерфейс — меню как в доставке еды: белый фон, реальные фото, карточка товара по тапу.

## Что где

| Файл | Зачем |
|---|---|
| `index.html`, `app.css`, `app.js` | само приложение (статика для GitHub Pages) |
| `config.js` | единственный файл с настройками: сюда вписывается адрес API |
| `api.js` | слой данных: продакшн — Apps Script, пусто — демо-режим из `hub-bar-data.json` |
| `img/*.webp` | фото напитков с вырезанным фоном, имя файла = id товара |
| `hub-bar-data.json` | данные: товары, подсчёты, закупки |
| `apps-script/` | серверная часть: `Code.gs` (API), `Seed.gs` (первичная заливка) |
| `hub-bar-prototype.html` | исходный прототип, оставлен как эталон логики |

## Демо-режим

Пока `config.js → API` пустой, приложение читает `hub-bar-data.json` и держит правки
в localStorage браузера. Удобно смотреть и щёлкать, но команда так работать не может —
у каждого будет свой склад.

Локально:

```bash
python3 -m http.server 8732
```

## Боевой контур

1. **Таблица.** Создать Google Таблицу на horovod.info@gmail.com, назвать «HOROVOD HUB · бар».
2. **Скрипт.** Расширения → Apps Script. Вставить `apps-script/Code.gs` и `apps-script/Seed.gs`.
3. **Свойства.** Project Settings → Script Properties:
   - `BOT_TOKEN` — токен бота @hrvdbarbot (единственное обязательное);
   - `ADMIN_IDS` — telegram id через запятую, необязательно.
4. **Заливка.** Выполнить функцию `setup()` — создаст листы `products`, `counts`, `purchases`, `team`
   и зальёт данные. Проверить `selfTest()`.
5. **Деплой.** Deploy → New deployment → Web app: Execute as **Me**, Who has access **Anyone**.
   Скопировать URL вида `https://script.google.com/macros/s/…/exec`.
6. **Связать.** Вписать этот URL в `config.js → API`, запушить на GitHub Pages.
7. **Команда.** Лист `team` создаётся пустым. Первый, кто откроет приложение через Telegram,
   автоматически записывается туда админом — дальше пускает только тех, кто есть в листе.
   Поэтому открыть приложение первым должен ты. Остальных добавлять строками:
   telegram id, имя, роль (id человек узнаёт у @userinfobot).
8. **Бот.** BotFather → `/mybots` → @hrvdbarbot → Bot Settings → Menu Button → адрес GitHub Pages.

Проверка API без Telegram (должна вернуть `ok:false` с просьбой открыть через Telegram —
значит, защита работает):

```bash
curl -s -X POST -H 'Content-Type: text/plain' -d '{"action":"list"}' "ВСТАВЬ_URL_ДЕПЛОЯ"
```

## Как считается

- **Выпито** = прошлый подсчёт + закупки − текущий подсчёт.
- **Ожидаемые деньги** = выпито платных × 1,50 €.
- **Расчётный остаток** между подсчётами = последний подсчёт + закупки − средний расход × прошедшие дни.
- **Средний расход** — по всем периодам, где позиция реально двигалась. Если между подсчётами её
  не покупали и остаток не изменился, это не «ноль расхода», а «нет данных» — так и написано в карточке.
- **Красный** — запаса меньше чем на 14 дней, **жёлтый** — меньше чем на 21.
- **«Что купить»** — на 28 дней вперёд, с округлением до упаковки.
- Цена закупки хранится **с залогом 0,15 €**, маржа считается от неё.

## Фото

Имя файла = id товара: `img/kozel05.webp`. Если файла нет, рисуется силуэт тары —
ломаться ничего не будет. Фото — packshot'ы производителей и магазинов с вырезанным фоном,
для внутреннего инструмента команды.

## Боевые адреса

| Что | Где |
|---|---|
| Приложение | https://horovodart.github.io/hrvdbarbot/ |
| Бот | @hrvdbarbot (кнопка меню «Бар») |
| API | Apps Script Web App, URL в `config.js` |
| Таблица | Google Sheets `1EAp62lw_p1MLIDL_vFgPVupboA7DosjZ3afDUblJ6LQ` на horovod.info@gmail.com |

Данные в таблицу заливаются сами при первом обращении из Telegram. Первый, кто откроет
приложение, записывается в лист `team` админом.

## Что дальше

**Загрузка чека прямо в приложении.** Сейчас чеки разбираются в переписке: Миша
присылает фото, оно читается, цены обновляются, новые товары заводятся, закупка
записывается. Работает, но упирается в одного человека.

Развитие: кнопка «загрузить чек» → фото уходит в Apps Script → оттуда в модель →
разбор возвращается предпросмотром (что за товары, какие цены, что новое) → человек
подтверждает, и только тогда пишется в таблицу. Тогда чек сможет завести любой из
команды.

Чего это требует и почему не сделано сразу:

- ключ модели в Script Properties — появляется счёт за разбор;
- разрешение `UrlFetchApp` для скрипта. Новое разрешение означает, что доступ
  придётся выдать заново, иначе приложение встанет **у всей команды** — этот риск
  уже всплывал в работе, и ровно поэтому серверные обращения наружу из кода убраны;
- предпросмотр с правкой до записи: разбор чека ошибается, и молча писать в таблицу
  нельзя.

Альтернатива без интерфейса загрузки — фото боту @hrvdbarbot, дальше то же самое.
Требования те же.

## Деплой

Одна команда — она же и есть единственный правильный способ выкатки:

```bash
./deploy.sh "что поменяли"
```

По шагам: гоняет все тесты → штампует версию на `js/css` → коммит и пуш на
GitHub Pages → `clasp push` и `clasp deploy` на **тот же** адрес `/exec`, чтобы
`config.js` не трогать → обновляет адрес мини-приложения в боте → дёргает живой
API и проверяет, что он отвечает. Если падает любой шаг — деплой останавливается,
а не доезжает наполовину.

### Про кэш Telegram — это кусалось

Версии на `js/css` спасают только их самих. Сам `index.html` Telegram держит в
кэше намертво, и никакая метка внутри файла этого не меняет: пока адрес
мини-приложения тот же, WebView может поднять старую страницу — со старым
`config.js`. А самый первый релиз ушёл на Pages с **пустым** `API`, то есть в
демо-режиме. Такая страница из кэша молча читает `localStorage` вместо сервера
и показывает пустой склад, ничем не выдавая подмены: ни ошибки, ни «нет связи».

Поэтому:

- деплой меняет адрес кнопки бота на `…/?v=<коммит>` — единственный способ
  заставить Telegram перечитать страницу. Для этого нужен файл `.env.local`
  (в git не попадает) со строкой `BOT_TOKEN=…`; без него шаг просто пропускается;
- на боевом хосте пустой `API` теперь ошибка с понятным текстом, а не тихий
  демо-режим;
- при живом `API` остатки демо-состояния в браузере стираются на старте.

Без аргумента выкатывает то, что уже закоммичено. Если в дереве есть правки, а
сообщение не передано — откажется, чтобы не утащить в прод случайное.

## Тесты

```bash
./tools/test.sh          # всё разом, 500 случаев фаззинга
./tools/test.sh 3000     # длинный прогон
```

Четыре набора:

| набор | что проверяет |
|---|---|
| `tools/tests/front/` | `model()` целиком: заморозка сумм, короткие периоды, амнистия, планка, тара, статусы, форматирование |
| `tools/tests/server/` | `Code.gs` на моках Google-окружения: синк, `auth`, замки, защита от формул, число обращений к листам |
| `tools/tests/fuzz/` | случайные склады против `tools/reference.py` — второй, независимой реализации той же логики |
| `tools/tests/fuzz/minimal.mjs` | регрессии на уже починенные расхождения |

`tools/reference.py` — не тест, а вторая реализация расчётов, написанная отдельно.
Смысл в том, что фаззер сверяет их между собой: если приложение и эталон разошлись,
врёт кто-то один, и это находка. **Правя логику в `app.js`, правь и эталон** — иначе
сверка тихо перестанет ловить ошибки именно там, где их дороже всего пропустить.

Тесты гоняются и сами — на каждый `git push`:

```bash
./tools/hooks/install.sh   # поставить хук (один раз на машину)
```

Хук лежит в `.git/hooks/`, а он не копируется вместе с репозиторием — поэтому
на новой машине его надо поставить заново. Осознанно обойти: `git push --no-verify`.

### Если правишь Apps Script

`clasp` уже настроен, руками в редактор ничего вставлять не надо — `./deploy.sh`
сам делает `push` и `deploy` на прежний адрес. Отдельно, без выкатки фронтенда:

```bash
./node_modules/.bin/clasp push --force
./node_modules/.bin/clasp deployments    # посмотреть адреса
```

Новый `deploy` **без** `--deploymentId` создаёт новый адрес `/exec` — тогда
придётся править `config.js` и переустанавливать ссылку в боте. `deploy.sh`
всегда выкатывает на прежний, так что этой ловушки можно не касаться.
